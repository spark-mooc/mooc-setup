...
# ANSWER
total_req_per_day_df = logs_df.groupBy(dayofmonth("time").alias("day")).count().orderBy("day")

avg_daily_req_per_host_df = (
  total_req_per_day_df.join(daily_hosts_df, total_req_per_day_df.day == daily_hosts_df.day)
                      .select(daily_hosts_df.day, (total_req_per_day_df[1] / daily_hosts_df[1]).alias("avg_reqs_per_host_per_day"))
                      .cache()
)
...
days_with_avg = []
avgs = []
for row in avg_daily_req_per_host_df.collect():
  days_with_avg.append(row.day)
  avgs.append(row.avg_reqs_per_host_per_day)
...
hosts_404_count_df = not_found_df.groupBy('host').count().orderBy('count', ascending=False)
...
hours_with_not_found = []
not_found_counts_per_hour = []
for row in hour_records_sorted_df.collect():
