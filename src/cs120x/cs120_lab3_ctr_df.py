...
# ANSWER
def one_hot_encoding(rawFeats, ohe_dict_broadcast, num_ohe_feats):
    """Produce a one-hot-encoding from a list of features and an OHE dictionary.
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indices equal to the unique
            ...
            with values equal to 1.0.
    """
    oheFeat = [ohe_dict_broadcast.value[(ind, category)] for ind, category in rawFeats]
    return SparseVector(num_ohe_feats, sorted(oheFeat), [1.] * len(oheFeat))

# Calculate the number of features in sample_OHE_dict_manual
num_sample_ohe_feats = len(sample_OHE_dict_manual.keys())
sample_ohe_dict_manual_broadcast = sc.broadcast(sample_OHE_dict_manual)

# Run oneHotEnoding on sample_one
...
def create_one_hot_dict(input_df):
    ...
    Args:
        input_df (DataFrame with 'feature' column): A DataFrame where each row contains a list of
...
def create_one_hot_dict(input_df):
    ...
    distinct_feats_df = (input_df
                           .select(explode(input_df.columns[-1]))
                           .distinct())
    key_value_tuple_list = (distinct_feats_df
                             .rdd
                             .map(lambda r: r[0])
                             .zipWithIndex()
                             .collect())
...
def parse_point(point):
    ...
    """
    values = point.split(',')
    return [(ind, val) for ind, val in enumerate(values[1:])]
...
def parse_raw_df(raw_df):
    ...
    """
    return raw_df.select(split('text', ',').getItem(0).cast('double').alias('label'),
                         parse_point_udf('text').alias('tuples')).cache()
...
ohe_train_df = (parsed_train_df
                  .select('label', ohe_dict_udf('tuples').alias('features'))
                  .cache())
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indices equal to the unique
            ...
            with values equal to 1.0.
    """
    ohe_feat = [ohe_dict_broadcast.value[(ind, category)] for ind, category in raw_feats
                if ohe_dict_broadcast.value.has_key((ind, category))]
    return SparseVector(num_ohe_feats, sorted(ohe_feat), [1.] * len(ohe_feat))

ohe_dict_missing_udf = ohe_udf_generator(ohe_dict_broadcast)
ohe_validation_df = (parse_raw_df(raw_validation_df)
                       .select('label', ohe_dict_missing_udf('tuples').alias('features'))
                       .cache())
...
log_loss_tr_base = (add_log_loss(ohe_train_df.withColumn('p', lit(class_one_frac_train)))
                 .selectExpr('avg(log_loss) as avg').first()[0])
print 'Baseline Train Logloss = {0:.3f}\n'.format(log_loss_tr_base)
...
def evaluate_results(df, model, baseline=None):
    ...
    Returns:
        float: Log loss for the data.
    """
    with_probability_df = df.withColumn('p', lit(baseline)) if baseline else add_probability(df, model)
    with_log_loss_df = add_log_loss(with_probability_df)
    log_loss = with_log_loss_df.selectExpr('avg(log_loss) as avgLogLoss').first()[0]
    return log_loss
...
log_loss_val_model_basic = evaluate_results(ohe_validation_df, lr_model_basic)

print ('OHE Features Validation Logloss:\n\tBaseline = {0:.3f}\n\tLogReg = {1:.3f}'
       .format(log_loss_val_base, log_loss_val_model_basic))
...
Test.assertTrue(np.allclose(log_loss_val_model_basic, expected_val_model_basic),
                'incorrect value for log_loss_val_model_basic. Got {0}, expected {1}'.format(log_loss_val_model_basic, expected_val_model_basic))
...
def add_hashed_features(df):
    ...
    """
    return (df
            .select('label', tuples_to_hash_features_udf('tuples').alias('features'))
            .cache())
...
def get_sparsity(df):
    ...
    """
    return (df
            .select(feature_sparsity_udf('features').alias('sparsity'))
            .selectExpr('avg(sparsity)')
            .first()[0])
