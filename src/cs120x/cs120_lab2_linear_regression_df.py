...
def parse_point(df):
    ...
    """
    return (df
             .select(sql_functions.split('value', ','))
             .rdd
...
# ANSWER
parsed_data_df = (parsed_points_df
                   .select('features', sql_functions.col('label') - min_year)
...
def get_labeled_prediction(weights, observation):
    ...
    Returns:
        tuple: A (prediction, label) tuple. Convert the return type of the prediction to a float.
    """
    return (float(weights.dot(observation.features)), observation.label)
...
# ANSWER  # Used to prevent this from showing up in student's notebooks
# Perform grid search on a larger set of hyperparameters.  Will take some time.
# No need to run, as precomputed values are stored in the next cell.
...
for alpha in alpha_params:
   for reg in reg_params:
      ...
      val_pred_df = model.transform(parsed_val_data_df)
      rmse_val_grid = calc_RMSE(val_pred_df)
      results.append((num_iters, reg, rmse_val_grid))

iters, reg, rmse_val = zip(*results) # retrieve results from grid search
...
def two_way_interactions(lp):
    ...
    new_features = map(lambda (x, y): x * y, itertools.product(features, repeat=2))
    return LabeledPoint(lp.label, np.hstack((features, np.array(new_features))))

print two_way_interactions(LabeledPoint(0.0, [2, 3]))

# Transform the existing train, validation, and test sets to include two-way interactions.
train_data_interact_df = (parsed_train_data_df
                           .rdd
                           .map(lambda lp: two_way_interactions(lp))
                           .toDF())
val_data_interact_df = (parsed_val_data_df
                         .rdd
                         .map(lambda lp: two_way_interactions(lp))
                         .toDF())
test_data_interact_df = (parsed_test_data_df
                          .rdd
                          .map(lambda lp: two_way_interactions(lp))
                          .toDF())
